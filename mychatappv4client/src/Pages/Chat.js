    ...
    const fetchMessages = async()=>
    {
        if(loggedInUser && selectedUser)
        {
            try
            {
                ...
                if(token)
                {
                    const response = await axios.get(`${process.env.REACT_APP_API_SERVER_URL}/api/messages?sender=${loggedInUser}&receiver=${selectedUser}`,
    ...
    useEffect(()=>  // WebSocket connection setup
    {
        const connect = ()=>
        {
            try
            {
                ...
                const socket = new SockJS(`${process.env.REACT_APP_API_SERVER_URL}/ws`);
